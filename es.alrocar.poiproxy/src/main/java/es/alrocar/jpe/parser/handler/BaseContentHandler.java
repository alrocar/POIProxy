...
public class BaseContentHandler {
	...
	for (String destProp : fType.getElements().keySet()) {
		...
		if (writerContentHandler != null)
		    writerContentHandler.addElementToFeature(
			    service.encode(arg0.toString()), destProp,
			    this.currentFeatureGeoJSON);

		// System.out.println(destProp + " --- " + arg0);
		contentHandler.addElementToFeature(
			service.encode(arg0.toString()), destProp,
			this.currentFeature);
		return;
	    }
	...
	return handler.addElementToFeature(service.encode(service.getId()),
		JPEParser.SERVICE, feature);
    }

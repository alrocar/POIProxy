...
* ***Search by tile***
     ...
     `http://localhost:8080/poiproxy/browse?service=panoramio&z=0&x=0&y=0&query=search_term&callback=whatever`

* ***Search by extent***

     Returns the points inside the given bounding box for the given query. The coordinate reference system is EPSG:4326

     `http://localhost:8080/poiproxy/browseByExtent?service=panoramio&minX=-0.376&minY=39.47&maxX=-0.37&maxY=39.48&query=search_term&callback=whatever`
...
* ***Search by lon,lat and distance***
     ...
     `http://localhost:8080/poiproxy/browseByLonLat?service=panoramio&lon=-0.38&lat=39.46&dist=500&query=search_term&callback=whatever`
